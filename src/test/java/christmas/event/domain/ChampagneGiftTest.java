...
class ChampagneGiftTest {
    ...
    void isEligible(Order order, VisitDate visit) {
        ChampagneGift champagneGift = new ChampagneGift();

        assertThat(champagneGift.isEligible(visit, order)).isTrue();
    }
    ...
    void isEligible_false(Order order, VisitDate visit) {
        ...
        assertThat(champagneGift.isEligible(visit, order)).isFalse();
    }
    ...
    private static Stream<Arguments> provideValidInput() {
        ...
        List<Integer> dates = Arrays.asList(1, 10, 25, 31);
